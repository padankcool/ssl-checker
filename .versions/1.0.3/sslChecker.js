  ...
  return new Promise((resolve) => {
    ...
    const socket = tls.connect(socketOptions, () => {
      try {
        ...
        const isOkNow = certStatus === 'valid' && !isExpiredNow && (daysRemaining === null || daysRemaining > 0);
        const sslStatus = isOkNow ? (isSafeNextYear ? 'OK' : 'Belum Diperbarui') : 'Error';
 ...
 * Menjamin hasil selalu berurutan sesuai dengan urutan input yang dimasukkan user
...
async function inspectBatch(domainList, concurrency = 8, onProgress = null) {
  ...
  // Cache periksa domain agar jika ada input duplikat, tidak melakukan koneksi TLS ganda
  const cache = new Map();

  const queue = domainList.map((domain, index) => ({ domain, index }));
  let completed = 0;

  async function worker() {
    while (queue.length > 0) {
      const item = queue.shift();
      const trimmed = (item.domain || '').trim();

      let resPromise;
      if (cache.has(trimmed)) {
        resPromise = cache.get(trimmed);
      } else {
        resPromise = inspectSSL(trimmed);
        cache.set(trimmed, resPromise);
      }

      const res = await resPromise;
      // Simpan di indeks yang sama persis sesuai urutan input asli
      results[item.index] = { ...res, input: item.domain };
      completed++;
      if (onProgress) {
        onProgress({ completed, total, currentResult: res, index: item.index });
      }
    }
  }
