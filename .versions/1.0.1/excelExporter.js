...
async function generateSSLWorkbook(data) {
  ...
  worksheet.columns = [
    ...
    { header: 'Status SSL', key: 'sslStatus', width: 16 },
  ...
  data.forEach((item, index) => {
    // Tentukan status SSL: 2 pilihan antara OK atau Error
    const isOk = item.sslStatus
      ? item.sslStatus === 'OK'
      : (item.status === 'valid' && !item.error && (item.daysRemaining === null || item.daysRemaining > 0));
    const sslStatusValue = isOk ? 'OK' : 'Error';
    ...
    sslStatusCell.dataValidation = {
      ...
      error: 'Status SSL hanya boleh dipilih OK atau Error'
    };
